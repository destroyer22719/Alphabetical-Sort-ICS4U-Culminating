...
describe("To test the outputToFile function", () => {
    test("output to txt", () => {
        ...
        expect(fs.readFileSync(path.join(outputDir, "output1.txt"), "utf8").split("\n")).toEqual(output1);
    
        outputToFile(output2, path.join(outputDir, "output2.txt"));
        expect(fs.readFileSync(path.join(outputDir, "output2.txt"), "utf8").split("\n")).toEqual(output2);
    
        outputToFile(output3, path.join(outputDir, "output3.txt"));
        expect(fs.readFileSync(path.join(outputDir, "output3.txt"), "utf8").split("\n")).toEqual(output3);
    });
    ...
    test("output to JSON", () => {
        ...
        expect(JSON.parse(fs.readFileSync(path.join(outputDir, "output1.json"), "utf8"))).toEqual(output1);
    
        outputToFile(output2, path.join(outputDir, "output2.json"));
        expect(JSON.parse(fs.readFileSync(path.join(outputDir, "output2.json"), "utf8"))).toEqual(output2);
    
        outputToFile(output3, path.join(outputDir, "output3.json"));
        expect(JSON.parse(fs.readFileSync(path.join(outputDir, "output3.json"), "utf8"))).toEqual(output3);
    });
});
